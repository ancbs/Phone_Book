...
#define Name_Size 30
#define Num_Size 10
// ure maintaining details about personal and proffessional contacts
class Tree_Node
{
public:
...
public:
    ...
    void Contact_Intializer(Tree_Node *ptr, char *name, long long int mob_num, char *email, char *office_num, char *address, char *office, int type)
    {
        ptr->name = name;
        ptr->mob_num = mob_num;
        ptr->email = email;
    ...
    void Print_Contact(Tree_Node *ptr)
    {
        if (ptr->type == 1)
        {
            printf("Professional\n");
    ...
    Tree_Node *left_rotate(Tree_Node *y)
    {
        Tree_Node *x;
        if (y->right == NULL)
        {
    ...
    int Balance_Factor(Tree_Node *ptr)
    {
        int retval;
        if (ptr == NULL)
        {
    ...
    Tree_Node *insert(Tree_Node *root, char *name, long long int mob_num, char *email, char *office_num, char *address, char *office, int type)
    {
    ...
    void Copy_Contact(Tree_Node *ptr1, Tree_Node *ptr2)
    {
        if (ptr2->type == 0)
        {
    ...
    Tree_Node *Inorder_Successor(Tree_Node *node)
    {
    ...
    Tree_Node *Delete(Tree_Node *root, long long int mob_num)
    {
        if (root != NULL)
        {
            if (mob_num < root->mob_num)
            ...
            else
            {
                ...
                {
                    Tree_Node *temp = Inorder_Successor(root);
                    Copy_Contact(root, temp);
    ...
    void Reverse_InOrder(Tree_Node *Root)
    {
        if (Root != NULL)
        {
    ...
    void Edit(Tree_Node *ptr, char *name, long long int mob_num, char *email, char *office_num, char *address, char *office)
    {
        ...
        if (ptr->type == 1)
        {
            ...
            if (mob_num != 0)
            {
                ...
                this->prof_root = Delete(this->prof_root, ptr->mob_num);
                this->prof_root = insert(this->prof_root, temp->name, mob_num, temp->email, temp->office_num, temp->address, temp->office, temp->type);
            }
        ...
        else
        {
            ...
            if (mob_num != 0)
            {
                Tree_Node *temp = (Tree_Node *)malloc(sizeof(Tree_Node));
                Copy_Contact(temp, ptr);
                this->per_root = insert(this->per_root, temp->name, mob_num, temp->email, temp->office_num, temp->address, temp->office, temp->type);
                this->per_root = Delete(this->per_root, ptr->mob_num);
            }
            printf("Modified Contact \n");
        }
    }
    // Function to display the nodes in sorted mannner(increasing,decreasing) based on the option provided
    void Sort(Tree_Node *root, int opt)
    {
        switch (opt)
        {
        case 1:
    ...
    void Range_Search(Tree_Node *root, long long int start, long long int end)
    {
        if (root != NULL)
        {
    ...
    Tree_Node *name_based_insert(Tree_Node *root, char *name, long long int mob_num, char *email, char *office_num, char *address, char *office, int type)
    {
        if (root == NULL)
        {
            root = (Tree_Node *)malloc(sizeof(Tree_Node));
    ...
    Tree_Node *name_based_sort(Tree_Node *name_root, Tree_Node *num_root)
    {
        if (num_root != NULL)
        {
    ...
    void Display_Name_Increasing(Tree_Node *root)
    {
        Tree_Node *nptr = root;
...
int main()
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (type == 0)
        {
            ...
            scanf("%s", email);
            phn_bk.per_root = phn_bk.insert(phn_bk.per_root, name, mob_num, email, NULL, NULL, NULL, type);
            // Display_All(0);
        }
        else
        {
            ...
            scanf("%s", office);
            phn_bk.prof_root = phn_bk.insert(phn_bk.prof_root, name, mob_num, email, office_num, address, office, type);
        }
    }
    int flag = 0;
    do
    {
        ...
        case 1:
            ...
            if (x == 0)
            {
                phn_bk.Edit(phn_bk.per_root, NULL, n, NULL, NULL, NULL, NULL);
            }
            else if (x == 1)
            {
                phn_bk.Edit(phn_bk.prof_root, NULL, n, NULL, NULL, NULL, NULL);
            }
        ...
        case 2:
            ...
            if (x == 0)
            {
                phn_bk.per_root = phn_bk.Delete(phn_bk.per_root, n);
            }
            else if (x == 1)
            {
                phn_bk.prof_root = phn_bk.Delete(phn_bk.prof_root, n);
            }
        ...
        case 4:
            ...
            if (x == 0)
            {
                ...
                scanf("%d", &y);
                phn_bk.Sort(phn_bk.per_root, y);
            }
            else if (x == 1)
            {
                printf("Enter 1 to sort in increasing order and  -1 to sort in decreasig order\n");
                scanf("%d", &y);
                phn_bk.Sort(phn_bk.prof_root, y);
            }
        ...
        case 6:
            ...
            if (x == 0)
            {
                phn_bk.Range_Search(phn_bk.per_root, n1, n2);
            }
            else if (x == 1)
            {
                phn_bk.Range_Search(phn_bk.prof_root, n1, n2);
            }
            else
            {
                printf("Enter the valid type\n");
        ...
        case 7:
            ...
            if (x == 0)
            {
                name_root = phn_bk.name_based_sort(name_root, phn_bk.per_root);
                phn_bk.Display_Name_Increasing(name_root);
            }
            else if (x == 1)
            {
                name_root = phn_bk.name_based_sort(name_root, phn_bk.prof_root);
                phn_bk.Display_Name_Increasing(name_root);
            }
            else
            {
